...
class ImageGroundViewModel (application: Application) : AndroidViewModel(application) {
    ...
    fun createScanTest(transcribedText: String, translatedText: String, savedFilePath: String){
        val scan = Scan(getCurrentDateTime(), getCurrentDate(), getCurrentTime(), "image", transcribedText, "Im not sure, This is not really a test translation", "entities", "smartReplies",
            savedFilePath, "audioUrl", "barcodeScan")
        addScan(scan)
    }
